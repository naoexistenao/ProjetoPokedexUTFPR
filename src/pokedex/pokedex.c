...
void inserir(Pokemon pokedex[], int *num_pokemons_alocacao){
    ...
    Pokemon *novo_pokemon = &pokedex[*num_pokemons_alocacao];
    //Recebendo o nome do pokemon
        printf("Digite o nome do pokemon: ");
        setbuf(stdin, NULL);
        fgets(novo_pokemon->nome_pokemon,20,stdin);
        novo_pokemon->nome_pokemon[strcspn(novo_pokemon->nome_pokemon,"\n")] = '\0';
        setbuf(stdin, NULL);

    //id
        novo_pokemon->id = *num_pokemons_alocacao;

    //tipo1
        printf("Digite o 1º tipo do seu pokemon: ");
        fgets(novo_pokemon->tipo1,10,stdin);
        novo_pokemon->tipo1[strcspn(novo_pokemon->tipo1, "\n")] = '\0';
        setbuf(stdin, NULL);

    //tipo2
        printf("Digite o 2º tipo do seu pokemon: ");
        fgets(novo_pokemon->tipo2,10,stdin);
        novo_pokemon->tipo2[strcspn(novo_pokemon->tipo2, "\n")] = '\0';
        setbuf(stdin, NULL);

    //vida
        printf("Digite a vida do pokemon: ");
        scanf("%d", &novo_pokemon->vida);

    //ataque
        printf("Digite o valor de ataque do pokemon: ");
        scanf("%d", &novo_pokemon->ataque);

    //defesa
        printf("Digite o valor da defesa: ");
        scanf("%d", &novo_pokemon->defesa);

    //ataque_especial
        printf("Digite o valor do ataque especial: ");
        scanf("%d", &novo_pokemon->ataque_especial);

    //defesa_especial
        printf("Digite o valor da defesa especial: ");
        scanf("%d", &novo_pokemon->defesa_especial);

    //velocidade
        printf("Digite o valor da velocidade do pokemon: ");
        scanf("%d", &novo_pokemon->velocidade);

    //geração
        printf("Digite a geração do pokemon: ");
        scanf("%d", &novo_pokemon->geracao);

    //lendário (1 se for lendário e 0 se não for(boleano))
        printf("Digite 1 se o pokemon for lendário e 0 se não for: ");
        scanf("%d", &novo_pokemon->lendario);

    //cor[10]
        setbuf(stdin, NULL);
        printf("Digite a cor do pokemon: ");
        fgets(novo_pokemon->cor, 10, stdin);
        novo_pokemon->cor[strcspn(novo_pokemon->cor, "\n")] = '\0';

    //altura float
        printf("Digite a altura do pokemon: ");
        scanf("%f", &novo_pokemon->altura);
        
    //peso float
        printf("Digite o peso do pokemon: ");
        scanf("%f", &novo_pokemon->peso);
        
    //taxa de captura
        printf("Digite a taxa de captura: ");
        scanf("%d", &novo_pokemon->taxa_captura);

    //total (soma = ataque, ataque_especial, defesa, defesa_especial, velocidade e vida)
        novo_pokemon->total = novo_pokemon->ataque + novo_pokemon->ataque_especial + novo_pokemon->defesa + novo_pokemon->defesa_especial + novo_pokemon->velocidade + novo_pokemon->vida;

        pokedex[*num_pokemons_alocacao] = *novo_pokemon;
...
//     for(int i = 1; i < num_pokemons_alocacao; i++){
//         if(strcasecmp(pokedex[i].nome_pokemon, pesquisar) == 0){
